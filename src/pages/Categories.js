...
export default function Categories(props) {
	...
	useEffect(
		function () {
			axios
				.get("https://api.spotify.com/v1/browse/categories?country=us", {
					headers: {
			...
			if (props.id) {
				axios
					.get(`https://api.spotify.com/v1/browse/categories/${props.id}/playlists?country=us`, {
						headers: {
							Authorization: "Bearer " + token.access_token,
						},
					})
				...
				<div className="categoryItems">
					{content.items?.map(function (result) {
						return (function () {
							if (props.id) {
								return (
									<CategorySubItem id={result.id} key={result.id}>
										{result.name}
									</CategorySubItem>
								);
							}
