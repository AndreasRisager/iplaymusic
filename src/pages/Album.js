...
export default function Album(props) {
	...
	useEffect(
		function () {
			if (props.id) {
				axios
					.get(`https://api.spotify.com/v1/albums/${props.id}`, {
						headers: {
				...
				<header className="albumHeader">
					...
					<div className="albumHeader__content">
						...
						<div className="albumHeader__contentText">
							<h1>{content.name}</h1>
							<h2>{content.total_tracks === 1 ? `${content.total_tracks} Song` : `${content.total_tracks} Songs`}</h2>
						</div>
					</div>
				</header>
						...
						{content.tracks?.items.map(function (result) {
							return (
								<Song
									...
									length={TimeCovert(result.duration_ms)}
									id={result.id}
									key={result.id}
								/>
							);
